...
# Host-aware ELF machine filter (readelf output)
host_uname_m=$(uname -m 2>/dev/null || echo "")
machine_re="Advanced Micro Devices X86-64|x86-64"
if [[ "$host_uname_m" == "aarch64" || "$host_uname_m" == "arm64" ]]; then
  machine_re="AArch64"
fi
...
while IFS= read -r -d '' f; do
  ...
  if [[ -z "$hdr" ]] || ! grep -qE "Class:\\s*ELF64" <<<"$hdr" || ! grep -qEi "Machine:\\s*(${machine_re})" <<<"$hdr"; then
    continue
  fi
  # Only executable
  [[ -x "$f" ]] || continue
