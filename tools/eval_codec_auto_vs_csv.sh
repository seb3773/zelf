...
usage(){
  echo "Usage: $0 --codec <lz4|apultra|zx7b|zx0|blz|pp|zstd|lzma|qlz|snappy|doboz|lzav|exo|shrinkler|lzsa|dsty> --csv <csv> [--out <out.csv>] [--limit N]" >&2
}
...
# Progress counters
# ------------------------------------------------------------
TOTAL_LINES=$(($(wc -l < "$CSV") - 1))
[[ $TOTAL_LINES -lt 0 ]] && TOTAL_LINES=0
if [[ $LIMIT -gt 0 && $LIMIT -lt $TOTAL_LINES ]]; then
  TOTAL_LINES=$LIMIT
fi
...
while IFS= read -r line || [[ -n "$line" ]]; do
  ...
  # Progress display
  # --------------------------------------------------------
  printf '\r[%d / %d] { %s }                          ' "$total" "$TOTAL_LINES" "$(basename -- "$path")" >&2
  ...
  pred=""
  pred=""
  # Relaxed grep to match any codec name in the log (e.g. DENSITY vs dsty)
  if echo "$LOG" | grep -Eqi "Heuristic +[A-Z0-9_]+ +filter +choice: *Kanzi"; then
    pred="kanziexe"
  elif echo "$LOG" | grep -Eqi "Heuristic +[A-Z0-9_]+ +filter +choice: *BCJ"; then
    pred="bcj"
  elif echo "$LOG" | grep -Eqi "Heuristic +[A-Z0-9_]+ +filter +choice:.*Kanzi.*chosen"; then
    pred="kanziexe"
  elif echo "$LOG" | grep -Eqi "Heuristic +[A-Z0-9_]+ +filter +choice:.*BCJ.*chosen"; then
    pred="bcj"
  ...
  if [[ "$winner" == "kanziexe" || "$winner" == "bcj" ]]; then
    [[ "$pred" == "$winner" ]] && correct=1
  fi

  [[ $correct -eq 1 ]] && ok=$((ok+1)) || ko=$((ko+1))
...
# New line after progress overwrite
echo >&2
...
acc="0.0"
if [[ $total -gt 0 ]]; then
  acc=$(awk -v ok="$ok" -v tot="$total" 'BEGIN{printf("%.4f", ok/tot)}')
fi
