...
/*
 * Modified to support files larger than 64KB:
 * - Changed load_located() and all load_*() functions to accept dynamic mem_size
 * - Added buffer overflow protection in load_raw()
 * - Removed hardcoded mem[65536] array size limitations
 */
...
/* Handles xex files */
static void load_xex(unsigned char *mem, int mem_size, FILE *in,
...
/* Handles tap files */
static void load_oric_tap(unsigned char *mem, int mem_size, FILE *in,
...
static void load_applesingle(unsigned char *mem, int mem_size, FILE *in,
                             struct load_info *load_info)
{
    int desc_size;
    int i;
 ...
 */
static void load_raw(unsigned char *mem, int mem_size, FILE *in,
                     ...
                     struct load_info *load_info)
{
    ...
    if (open_info->load_addr + open_info->length > mem_size)
    {
        LOG(LOG_ERROR,
            ("Error: File too large. Need %d bytes but buffer is only %d bytes.\n",
             open_info->load_addr + open_info->length, mem_size));
        fclose(in);
        exit(1);
    }
...
static int try_load_bbc_inf(unsigned char *mem, int mem_size,
                            FILE *in,
                            const char *filename,
                            struct open_info *open_info,
                            struct load_info *load_info)
{
    ...
    /* read data file here */
    open_info->load_addr = load;
    load_raw(mem, mem_size, in, open_info, load_info);
    if (load_info != NULL)
    {
...
void load_located(const char *filename, unsigned char *mem, int mem_size,
                  struct load_info *info)
{
    struct open_info open_info;
    FILE *in;
    ...
    else if (try_load_bbc_inf(mem, mem_size, in, filename, &open_info, info))
    {
        type = BBC_INF;
    }
    else
    ...
    case ATARI_XEX:
        /* file is an xex file */
        load_xex(mem, mem_size, in, info);
        break;
    case ORIC_TAP:
        /* file is an oric tap file */
        load_oric_tap(mem, mem_size, in, info);
        break;
    case APPLESINGLE:
        /* file is an AppleSingle file */
        load_applesingle(mem, mem_size, in, info);
        break;
    case BBC_INF:
    ...
    case RAW:
        ...
        load_raw(mem, mem_size, in, &open_info, info);
        break;
    default:
