...
/* Optional packer progress callback (defined in exo_compress_wrapper.c) */
extern void (*g_exo_progress_cb)(int);
extern int g_exo_pass_index;
extern int g_exo_max_passes;
extern int g_exo_last_global;
                      ...
                      struct buf *enc) /* IN */
{
    ...
    g_exo_pass_index = pass - 1; /* 0-based current pass */
    g_exo_max_passes = options->max_passes ? options->max_passes : 3;
    {
        int base = (g_exo_pass_index * 100) / g_exo_max_passes;
        if (g_exo_last_global < base) g_exo_last_global = base;
    }
    if (g_exo_progress_cb) {
        int mp = options->max_passes ? options->max_passes : 3;
        int pct = (int)(((pass - 1) * 100) / mp);
        if (pct < 0) pct = 0; if (pct > 99) pct = 99; /* leave 100% for final */
        g_exo_progress_cb(pct);
    }
    ...
    last_waltz:
        ...
        g_exo_pass_index = pass - 1; /* update current pass index */
        {
            int base = (g_exo_pass_index * 100) / (options->max_passes ? options->max_passes : 3);
            if (g_exo_last_global < base) g_exo_last_global = base;
        }
        if (g_exo_progress_cb) {
            int mp = options->max_passes ? options->max_passes : 3;
                  ...
                  struct crunch_info *infop) /* OUT */
{
    ...
    if (g_exo_progress_cb) g_exo_progress_cb(0);
    ...
    if (g_exo_progress_cb) g_exo_progress_cb(100);
}
