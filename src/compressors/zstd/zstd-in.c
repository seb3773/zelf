...
/* --- ELFZ packer minimization knobs --- */
/* We only need compression in the packer; decompression is handled by our stub. */
#define ZSTD_STRIP_ERROR_STRINGS 1
#define ZSTD_NO_UNUSED_FUNCTIONS 1
/* Force single-threaded build to avoid pulling threading/pool backends */
...
/* Packer does not need decompressor: excluded to reduce binary size */
#if defined(ELFZ_NEED_DECOMPRESS)
...
// --- BEGIN DISABLED OPTIMIZATION ---
// Disable dictBuilder sources for ELFZ packer to reduce code size and avoid
// conflicting type declarations when amalgamated. The packer does not build
// dictionaries at runtime; we only need compression/decompression.
#if 0
