...
typedef enum _lzsa_status_t {
  ...
  LZSA_ERROR_RAW_TOOLARGE,     /**< Input is too large to be compressed to a raw
                                  block */
  LZSA_ERROR_RAW_UNCOMPRESSED, /**< Input is incompressible and raw blocks don't
                                  support uncompressed data */

  /* Decompression-specific status codes */
  LZSA_ERROR_FORMAT,       /**< Invalid input format or magic number when
                              decompressing */
...
typedef struct _lzsa_stream_t {
   ...
   * @param stream stream
   *
   * @return nonzero if the end of the data has been reached, 0 if there is more
   * data
   */
 ...
 */
int lzsa_filestream_open(lzsa_stream_t *stream, const char *pszInFilename,
                         const char *pszMode);
