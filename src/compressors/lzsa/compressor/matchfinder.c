...
static inline int lzsa_get_index_tag(unsigned int nIndex) {
  return (int)(((unsigned long long)nIndex * 11400714819323198485ULL) >>
               (64ULL - TAG_BITS));
}
 ...
 */
int lzsa_build_suffix_array(lzsa_compressor *pCompressor,
  ...
  if (lzsa_divsufsort_build_array(&pCompressor->divsufsort_context, pInWindow,
                                  (saidx_t *)intervals, nInWindowSize) != 0) {
    return 100;
  }
  ...
  for (i = 0; i < nInWindowSize; i++) {
    ...
    const int nMaxLen =
        (i > Phi[i]) ? (nInWindowSize - i) : (nInWindowSize - Phi[i]);
    while (nCurLen < nMaxLen &&
           pInWindow[i + nCurLen] == pInWindow[Phi[i] + nCurLen])
      nCurLen++;
    PLCP[i] = nCurLen;
    if (nCurLen > 0)
      nCurLen--;
  ...
  /* Rotate permuted LCP into the LCP. This has better cache locality than the
   * direct Kasai LCP method. This also saves us from having to build the
   * inverse suffix array index, as the LCP is calculated without it using this
  ...
  if (pCompressor->format_version >= 2) {
    for (i = 1; i < nInWindowSize; i++) {
      ...
      if (nLen)
        nTaggedLen =
            (nLen << TAG_BITS) |
            (lzsa_get_index_tag((unsigned int)nIndex) & ((1 << TAG_BITS) - 1));
      intervals[i] =
          ((unsigned int)nIndex) | (((unsigned int)nTaggedLen) << LCP_SHIFT);
    }
  } else {
    for (i = 1; i < nInWindowSize; i++) {
      const int nIndex = (int)(intervals[i] & POS_MASK);
      ...
      intervals[i] =
          ((unsigned int)nIndex) | (((unsigned int)nLen) << LCP_SHIFT);
    }
  }
 ...
 */
static int lzsa_find_matches_at(lzsa_compressor *pCompressor, const int nOffset,
                                lzsa_match *pMatches, const int nMaxMatches,
                                const int nInWindowSize) {
  unsigned int *intervals = pCompressor->intervals;
  unsigned int *pos_data = pCompressor->pos_data;
  unsigned int ref;
  ...
  if (pCompressor->format_version >= 2 && nInWindowSize < 65536) {
    if ((matchptr - pMatches) < nMaxMatches) {
      const unsigned int nMatchOffset =
          (const unsigned int)(nOffset - match_pos);

      if (nMatchOffset <= MAX_OFFSET) {
        matchptr->length =
            (const unsigned short)(ref >> (LCP_SHIFT + TAG_BITS));
  ...
  for (;;) {
    ...
    if (pCompressor->format_version >= 2 && nInWindowSize < 65536) {
      if ((matchptr - pMatches) < nMaxMatches) {
        ...
        if (nMatchOffset <= MAX_OFFSET) {
          const unsigned short nMatchLen =
              ((const unsigned short)(ref >> (LCP_SHIFT + TAG_BITS)));
 ...
 */
void lzsa_skip_matches(lzsa_compressor *pCompressor, const int nStartOffset,
                       const int nEndOffset) {
  lzsa_match match;
  int i;

  /* Skipping still requires scanning for matches, as this also performs a lazy
   * update of the intervals. However, we don't store the matches. */
 ...
 */
void lzsa_find_all_matches(lzsa_compressor *pCompressor,
                           const int nMatchesPerOffset, const int nStartOffset,
                           const int nEndOffset) {
  lzsa_match *pMatch = pCompressor->match;
  int i;

  for (i = nStartOffset; i < nEndOffset; i++) {
    const int nMatches = lzsa_find_matches_at(
        pCompressor, i, pMatch, nMatchesPerOffset, nEndOffset - nStartOffset);

    if (nMatches < nMatchesPerOffset) {
      memset(pMatch + nMatches, 0,
             (nMatchesPerOffset - nMatches) * sizeof(lzsa_match));
    }
