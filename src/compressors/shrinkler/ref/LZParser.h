...
public:
    ...
    RefEdgeFactory(int edge_capacity) : edge_capacity(edge_capacity),
        edge_count(0), cleaned_edges(0), buffer(NULL), pool_mem(NULL), pool_off(0),
        max_edge_count(0), max_cleaned_edges(0)
    {
        if (edge_capacity > 0) {
            pool_mem = ::operator new((size_t)edge_capacity * sizeof(RefEdge));
        }
    }

    ~RefEdgeFactory() {
        // Delete heap-allocated edges still in the free list (non-pool)
        RefEdge* cur = buffer;
        char* base = (char*)pool_mem;
        size_t pool_bytes = (size_t)edge_capacity * sizeof(RefEdge);
        while (cur != NULL) {
            RefEdge* next = cur->source;
            if (!(pool_mem && (char*)cur >= base && (char*)cur < base + pool_bytes)) {
                // Was heap-allocated
                ::operator delete(cur);
            }
            cur = next;
    ...
    RefEdge* create(int pos, int offset, int length, int total_size, RefEdge *source) {
        ...
        if (pool_mem && pool_off < edge_capacity) {
            char* p = (char*)pool_mem + ((size_t)pool_off++ * sizeof(RefEdge));
            return new (p) RefEdge(pos, offset, length, total_size, source);
        }
