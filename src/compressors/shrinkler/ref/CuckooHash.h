...
template <typename V> class CuckooHashIterator {
  const CuckooHash<V> *table;
  int index;
  CuckooHashIterator(const CuckooHash<V> *table, int index)
      : table(table), index(index) {}

  void find() {
    while (table->element_array[index].first == CuckooHash<V>::UNUSED)
      index++;
  }
...
private:
  ...
  int array_size() const {
    const unsigned bits = (unsigned)(sizeof(hash_type) * 8u);
    const unsigned hs = (unsigned)hash_shift;
    if (__builtin_expect(hs > bits, 0))
      return 1;
    const unsigned exp = bits - hs;
    if (__builtin_expect(exp >= 30u, 0))
      return (int)(1u << 30);
    return (int)(1u << exp);
  }
...
public:
  ...
  ~CuckooHash() { delete[] element_array; }
  ...
  iterator begin() const { return CuckooHashIterator<V>(this, 0); }
  ...
  iterator end() const {
    ...
    while (index > 0 && array[index - 1].first == UNUSED)
      index--;
    return CuckooHashIterator<V>(this, index);
  }

  int size() const { return n_elements; }
  ...
  int count(int key) const {
    ...
    // assert(element_array != NULL);
    value_type *array = element_array;
    if (array[hash1].first == key || array[hash2].first == key)
      return 1;
    return 0;
  }
