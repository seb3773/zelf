...
// AVX-512BW path: 64 bytes per step
...
static inline int lcp_eq_runtime_u8(const unsigned char* a, const unsigned char* b, int max) {
    // Avoid __builtin_cpu_supports (pulls in __cpu_model at link time).
    // Use SSE2 path unconditionally (baseline on x86_64).
