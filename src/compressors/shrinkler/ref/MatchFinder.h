...
class MatchFinder {
	...
	__attribute__((hot)) void make_suffix_array() {
		...
		for (int i = 0 ; i < length ; i++) {
			...
			if (r < length) {
				...
				if (h < m) {
					// Prefetch ahead for both sides to reduce L1 miss rate during AVX compares
					int pf_a = i + h + 96;
					int pf_b = j + h + 96;
					if (pf_a < length) __builtin_prefetch(&data[pf_a], 0, 1);
					if (pf_b < length) __builtin_prefetch(&data[pf_b], 0, 1);
					h += lcp_eq_runtime_u8(&data[i + h], &data[j + h], m - h);
				}
	...
	void extend_left() {
		...
		while (left_length >= min_length) {
			// Prefetch next iteration's entries (if in range)
			int pf_idx = left_index - 1;
			if (pf_idx > 0) {
				__builtin_prefetch(&longest_common_prefix[pf_idx], 0, 1);
				__builtin_prefetch(&suffix_array[pf_idx], 0, 1);
			}
			...
			if (UNLIKELY(++iter > match_patience)) {
				left_length = 0;
				break;
			}
		}
	...
	void extend_right() {
		int iter = 0;
		while (true) {
			// Prefetch for the next loop turn
			int pf_lcp = right_index + 1;
			int pf_sa  = right_index + 2; // next SA index we will read
			if (pf_lcp <= length) __builtin_prefetch(&longest_common_prefix[pf_lcp], 0, 1);
			if (pf_sa  <= length) __builtin_prefetch(&suffix_array[pf_sa], 0, 1);
