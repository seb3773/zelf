...
// Induce step used by SA-IS
static inline void induce(const int * __restrict data, int * __restrict suffix_array, int length, int alphabet_size, const vector<bool>& stype, const int * __restrict buckets, int * __restrict bucket_index) {
    // Induce L suffixes
...
static inline int lcp_eq_runtime_u8(const unsigned char* a, const unsigned char* b, int max) {
    // Avoid relying on __builtin_cpu_supports (pulls in __cpu_model at link time).
    // On x86_64, SSE2 is baseline; use it unconditionally for deterministic builds.
    return lcp_eq_sse2_u8(a, b, max);
}
// --- END LCP U8 HELPERS ---

static inline bool substrings_equal(const int *data, int i1, int i2, const vector<bool>& stype) {
...
static inline void computeSuffixArray(const int * __restrict data, int * __restrict suffix_array, int length, int alphabet_size) {
    assert(length >= 1);
    if (length == 1) {
        suffix_array[0] = 0;
        return;
