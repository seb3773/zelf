...
extern "C" int shrinkler_compress_memory(const uint8_t *in, size_t in_size,
...
int shrinkler_compress_memory(const uint8_t *in, size_t in_size,
  ...
  long long margin_ll = (long long)verifier.front_overlap_margin +
                        (long long)pack_buffer.size() - (long long)in_size;
  if (margin_ll < 0)
    margin_ll = 0;
  if (margin_ll > 0x7fffffffLL)
    margin_ll = 0x7fffffffLL;
  unsigned margin_u32 = (unsigned)margin_ll;
