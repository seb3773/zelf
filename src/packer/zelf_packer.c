...
static const char *elf_machine_to_type_str(int e_machine) {
...
static int read_elf_machine_quick(const char *path, int *out_machine) {
  if (!path || !out_machine)
    return 0;
...
static void print_version_full(void) {
...
#endif
  const char *linkage =
      (strcmp(ZELF_BUILD_LINKAGE, "Static") == 0) ? "static" : "dynamic";
  fprintf(out, "%s%s %s build%s - %s%s\n", PKC(GREY17), host_arch, linkage,
          PK_RES, ZELF_BUILD_DATETIME, PK_RES);
  ...
  extern const unsigned char _binary_build_stage0_nrv2b_le32_x86_64_bin_start[];
  extern const unsigned char _binary_build_stage0_nrv2b_le32_x86_64_bin_end[];
  extern const unsigned char _binary_build_stage0_nrv2b_le32_arm64_bin_start[];
  extern const unsigned char _binary_build_stage0_nrv2b_le32_arm64_bin_end[];

  const int host_machine =
#if defined(__aarch64__)
      EM_AARCH64;
#else
      EM_X86_64;
#endif
  const int other_machine =
#if defined(__aarch64__)
      EM_X86_64;
#else
      EM_AARCH64;
  ...
  const size_t stage0_x86_sz =
      (size_t)(_binary_build_stage0_nrv2b_le32_x86_64_bin_end -
               _binary_build_stage0_nrv2b_le32_x86_64_bin_start);
  const size_t stage0_arm64_sz =
      (size_t)(_binary_build_stage0_nrv2b_le32_arm64_bin_end -
               _binary_build_stage0_nrv2b_le32_arm64_bin_start);

  const int has_stage0_other =
      (other_machine == EM_AARCH64) ? (stage0_arm64_sz != 0u)
                                    : (stage0_x86_sz != 0u);

  const unsigned char *s = NULL, *e = NULL;
  exe_filter_t saved = g_exe_filter;
  g_exe_filter = EXE_FILTER_BCJ;
  select_embedded_stub("lz4", /*mode=*/0, /*password=*/0, other_machine, &s, &e);
  g_exe_filter = saved;

  const int has_stub_other = (s && e && e > s);
  const int has_other = (has_stage0_other && has_stub_other);

  fprintf(out, "\nSupported binaries for packing:\n");
  if (host_machine == EM_AARCH64) {
    fprintf(out, " - arm64\n");
    if (has_other)
      fprintf(out, " - x86_64\n");
  } else {
...
static inline stage0_desc_t stage0_get_desc(int target_machine) {
  stage0_desc_t d;
  if (target_machine == EM_AARCH64) {
    d.s0 = _binary_build_stage0_nrv2b_le32_arm64_bin_start;
    d.s0_sz = (size_t)(_binary_build_stage0_nrv2b_le32_arm64_bin_end -
                       _binary_build_stage0_nrv2b_le32_arm64_bin_start);
    ...
    d.missing_name = "stage0_nrv2b_le32_arm64.bin";
  } else {
    d.s0 = _binary_build_stage0_nrv2b_le32_x86_64_bin_start;
    d.s0_sz = (size_t)(_binary_build_stage0_nrv2b_le32_x86_64_bin_end -
                       _binary_build_stage0_nrv2b_le32_x86_64_bin_start);
    ...
    d.missing_name = "stage0_nrv2b_le32_x86_64.bin";
  }
  return d;
}

static int build_zstd_stage0_blob(const unsigned char *stub, size_t stub_sz,
                                  int has_pt_interp, int target_machine,
                                  size_t packed_block_size,
                                  unsigned char **out_buf, size_t *out_filesz,
                                  size_t *out_memsz, uint8_t stage0_flags) {
  stage0_desc_t d = stage0_get_desc(target_machine);

  if (d.s0_sz == 0) {
    fprintf(stderr,
            "[%s%s%s] Error: stage0 blob not available (build missing prebuilt %s). "
            "Cannot use stage0-wrapped codec for this host.\n",
            PK_ERR, PK_SYM_ERR, PK_RES, d.missing_name);
    return -1;
  }
  ...
  if (has_pt_interp) {
    ...
    if (target_machine == EM_AARCH64) {
      // ARM64 dynamic stage0 entry is at vaddr 0x5100 (0x5000 + 0x100). The
      // decompressed stub base must keep low12==0 to preserve ADRP/add based
      // addressing without full PIE/relocations.
      // Therefore enforce: (dst_off + 0x100) % 0x1000 == 0.
      dst_off = align_up(filesz + packed_block_size + 0x100u, 0x1000u) - 0x100u;
    } else {
  ...
  memcpy(buf, d.s0, d.s0_sz);
  // Patch header (little-endian)
  {
    uint32_t v32;
    uint64_t v64;
    v32 = out_cap;
    memcpy(buf + d.ulen_off, &v32, sizeof(v32));
    v32 = clen;
    memcpy(buf + d.clen_off, &v32, sizeof(v32));
    v64 = (uint64_t)dst_off;
    memcpy(buf + d.dst_off, &v64, sizeof(v64));
    v64 = (uint64_t)blob_off;
    memcpy(buf + d.blob_off, &v64, sizeof(v64));
  }
  buf[d.flags_off] = stage0_flags;
...
static int build_density_stage0_blob(const unsigned char *stub, size_t stub_sz,
  ...
  if (has_pt_interp) {
    if (target_machine == EM_AARCH64) {
      // AArch64: decompressed stub base must keep low12==0 to preserve ADRP/add
      // addressing without full PIE/relocations.
      // Therefore enforce: (dst_off + 0x100) % 0x1000 == 0.
      dst_off = align_up(filesz + packed_block_size + 0x100u, 0x1000u) - 0x100u;
    } else {
...
static int compute_effective_size(const char *input_path, int no_strip,
  ...
  if (ehdr->e_machine != EM_X86_64 && ehdr->e_machine != EM_AARCH64) {
    fprintf(stderr, "[%s%s%s] Error: Only x86_64/arm64 ELF supported\n",
...
static int pack_elf_once(const char *input_path, const char *output_path,
  ...
  if (print_header) {
           ...
           input_path);
    {
      ...
      if (read_elf_machine_quick(input_path, &m))
        t = elf_machine_to_type_str(m);
      printf("[%s%s%s] ELF binary type: %s\n", PK_INFO, PK_SYM_INFO, PK_RES, t);
    }
  }
  ...
  // Check architecture (x86_64 or arm64)
  if (ehdr->e_machine != EM_X86_64 && ehdr->e_machine != EM_AARCH64) {
    fprintf(stderr,
            "[%s%s%s] Error: Only x86_64/arm64 ELF binaries are supported\n",
            PK_ERR, PK_SYM_ERR, PK_RES);
    goto cleanup;
  }

  const int target_machine = (int)ehdr->e_machine;
  ...
  select_embedded_stub(codec, stub_mode, use_password, target_machine,
                       &stub_start, &stub_end);
  ...
  if (g_exe_filter == EXE_FILTER_AUTO) {
    // TEMPORARY: AUTO filter selection is not reliable for AArch64 yet.
    // The current ML predictors are trained/tuned for x86 patterns. Until a
    // dedicated ARM64 dataset/model is collected+trained+distilled (after all
    // codecs are integrated), force AUTO -> BCJ for ARM64.
    if (target_machine == EM_AARCH64) {
      g_exe_filter = EXE_FILTER_BCJ;
      VPRINTF("[%s%s%s] AUTO filter (ARM64): forcing BCJ (temporary, pending ARM64 ML predictor)\n",
              PK_INFO, PK_SYM_INFO, PK_RES);
      ...
      VPRINTF("[%s%s%s] Stub re-selected (AUTO ARM64->BCJ): %zu bytes\n",
...
// If you don't like it, bite me!
  ...
  if (g_exe_filter == EXE_FILTER_KANZIEXE) {
    ...
    if (target_machine == EM_AARCH64) {
      if (text_off != (size_t)-1 && text_end > text_off) {
        int code_start = (int)text_off;
        int code_end = (int)text_end;

        if (has_pt_interp && strcmp(codec, "density") == 0 && text_off == 0) {
          size_t min_start = (size_t)ehdr->e_ehsize;
          size_t ph_end =
              (size_t)ehdr->e_phoff + (size_t)ehdr->e_phnum * (size_t)ehdr->e_phentsize;
          if (ph_end > min_start)
            min_start = ph_end;
          ...
          min_start &= ~3u;
          if (min_start < text_end)
            code_start = (int)min_start;

          if (g_verbose) {
            VPRINTF(
                "[%s%s%s] Kanzi ARM64 range (patched): codeStart=%d codeEnd=%d (text_off=%zu text_end=%zu)\n",
                PK_INFO, PK_SYM_INFO, PK_RES, code_start, code_end, text_off,
                text_end);
          }
        }

        fsz = kanzi_exe_filter_encode_force_arm64_range(
            combined, (int)actual_size, filtered_buf, max_sz, &processed,
            code_start, code_end);
      } else {
        fsz = kanzi_exe_filter_encode_arm64(combined, (int)actual_size,
                                            filtered_buf, max_sz, &processed);
      }
    } else {
      if (text_off != (size_t)-1 && text_end > text_off) {
    ...
    if (fsz > 0) {
      ...
      if (fatal_on_error)
        fail("KanziExe failed");
      fprintf(stderr, "[%s%s%s] Error: KanziExe failed\n", PK_ERR, PK_SYM_ERR,
              PK_RES);
      goto cleanup;
    }
    ...
    size_t done;
    if (target_machine == EM_AARCH64)
      done = bcj_arm64_encode(filtered_buf, actual_size, 0);
    else
      done = bcj_x86_encode(filtered_buf, actual_size, 0);
    if (done > 0) {
      data_to_compress = filtered_buf;
      ...
      if (fatal_on_error)
        fail("BCJ failed");
      fprintf(stderr, "[%s%s%s] Error: BCJ failed\n", PK_ERR, PK_SYM_ERR,
              PK_RES);
      goto cleanup;
    }
  }
  ...
  VPRINTF(
      ...
      PK_INFO, PK_SYM_EST, PK_RES, stub_size, comp_size,
  ...
  VPRINTF("[%s%s%s] stub_off=0x%lx, estimated_stub_sz=%zu, ALIGN=%d\n", PK_INFO,
          PK_SYM_EST, PK_RES, stub_off, estimated_stub_sz, ALIGN);
  ...
  if (strcmp(codec, "zstd") == 0) {
    ...
    if (build_zstd_stage0_blob(stub_patch, stub_size, has_pt_interp,
                               target_machine, packed_block_size, &wrapped,
                               &filesz, &memsz, elfz_flags) != 0) {
            ...
            PK_SYM_INFO, PK_RES, filesz, memsz);
    {
      ...
      memcpy(&ulen, wrapped + d.ulen_off, sizeof(ulen));
      memcpy(&clen, wrapped + d.clen_off, sizeof(clen));
      memcpy(&dst_off, wrapped + d.dst_off, sizeof(dst_off));
      memcpy(&blob_off, wrapped + d.blob_off, sizeof(blob_off));
      VPRINTF(
  ...
  // doesn't compress well enough with NRV2B to offset the stage0 overhead.
    ...
    unsigned char *wrapped = NULL;
    size_t filesz = 0, memsz = 0;
    // Density packed block layout depends on filter (same rule as ZSTD):
    // - None/BCJ => legacy layout (no filtered_size)
    // - KanziEXE => modern layout (filtered_size present)
    size_t packed_block_size =
        strlen(marker) + sizeof(size_t) + sizeof(uint64_t) + sizeof(int) +
        ((g_exe_filter == EXE_FILTER_KANZIEXE) ? sizeof(int) : 0) +
        ((target_machine == EM_AARCH64 && stub_mode == 2 &&
          g_exe_filter == EXE_FILTER_KANZIEXE)
             ? (sizeof(uint32_t) + 40u)
             : 0u) +
        (size_t)comp_size;
    ...
    memcpy(&actual_dst_off_u64, wrapped + d.dst_off,
           sizeof(actual_dst_off_u64));
    memcpy(&decompressed_size_u32, wrapped + d.ulen_off,
    ...
    {
      stage0_desc_t d2 = stage0_get_desc(target_machine);
      uint32_t ulen = 0, clen = 0;
      uint64_t dst_off = 0;
      memcpy(&ulen, wrapped + d2.ulen_off, sizeof(ulen));
      memcpy(&clen, wrapped + d2.clen_off, sizeof(clen));
      memcpy(&dst_off, wrapped + d2.dst_off, sizeof(dst_off));
      VPRINTF(
          "[%s%s%s] density stage0 header: ulen=%u clen=%u dst_off=0x%zx\n",
          PK_INFO, PK_SYM_INFO, PK_RES, (unsigned)ulen, (unsigned)clen,
          (size_t)dst_off);
      VPRINTF("[%s%s%s] density stage0 flags=0x%x\n", PK_INFO, PK_SYM_INFO,
              PK_RES, (unsigned)wrapped[d2.flags_off]);
    }
...
write_binary:
  ...
  if (target_machine == EM_AARCH64 && has_pt_interp &&
      stub_mode == 2 && strcmp(codec, "density") == 0 &&
      g_exe_filter == EXE_FILTER_KANZIEXE && filtered_buf != NULL &&
      filtered_size >= 40u && data_to_compress == filtered_buf &&
      size_to_compress == filtered_size) {
    density_kexe_tail40 = filtered_buf + (filtered_size - 40u);
  }
  ...
  rc = write_packed_binary(output_path, has_pt_interp, stub_start, stub_size,
                           ...
                           marker, elfz_flags, use_password, pwd_salt, pwd_obf,
                           (Elf64_Half)target_machine, density_kexe_tail40);
  if (rc != 0)
    goto cleanup;
...
// ** This mode will be extremely SLOW (all codecs, all filters), only for crazy people :)
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (strcmp(argv[i], "-best") == 0) {
      ...
      codec_explicit = 1;
               ...
               strcmp(argv[i], "-brieflz") == 0) {
      codec = "blz";
      codec_explicit = 1;
    } else if (strcmp(argv[i], "-exo") == 0 ||
               strcmp(argv[i], "-exomizer") == 0) {
      codec = "exo";
      codec_explicit = 1;
    } else if (strcmp(argv[i], "-pp") == 0 ||
               strcmp(argv[i], "-powerpacker") == 0) {
      codec = "pp";
      codec_explicit = 1;
      ...
      mode_help_faq = 1;
               ...
               strcmp(argv[i], "--helpcredits") == 0) {
      mode_help_credits = 1;
    } else if (strcmp(argv[i], "--no-colors") == 0 ||
               strcmp(argv[i], "--nocolors") == 0) {
      ...
      output_arg = argv[++i];
               ...
               strcmp(argv[i], "--no-backup") == 0) {
      no_backup = 1;
    } else if (strcmp(argv[i], "--archive") == 0) {
      mode_archive = 1;
    } else if (strcmp(argv[i], "--archive-tar") == 0 ||
               strcmp(argv[i], "--archivetar") == 0) { 
      mode_archive_tar = 1;
    } else if (strcmp(argv[i], "--archive-sfx") == 0 ||
               strcmp(argv[i], "--archivesfx") == 0) { 
      mode_archive_sfx = 1;
    } else if (strcmp(argv[i], "--archive-tar-sfx") == 0 ||
               strcmp(argv[i], "--archivetarsfx") == 0) {
      mode_archive_tar_sfx = 1;
    } else if (strcmp(argv[i], "--archive-info") == 0 ||
               strcmp(argv[i], "--archiveinfo") == 0) {
      mode_archive_list = 1;
  ...
  if (!mode_archive && !mode_unpack && !mode_best && !mode_archive_list) {
            ...
            input_path);
    {
      ...
      if (read_elf_machine_quick(input_path, &m))
        t = elf_machine_to_type_str(m);
      fprintf(out, "[%s%s%s] ELF binary type: %s\n", PK_INFO, PK_SYM_INFO,
              PK_RES, t);
    }
    ...
    FILE *out = g_ui_stream ? g_ui_stream : stdout;
    fprintf(out, "%s❖ Archive INFO mode%s\n", PKC(FYW), PK_RES);
  }

  // Handle Archive/Unpack modes (not allowed with -best :-p)
  if (mode_archive) {
