...
static inline void start_progress(const char *msg, int id) {
  if (!g_no_progress) start_progress_real(msg, id);
}
...
static inline void update_progress(int pct, int id) {
  if (!g_no_progress) update_progress_real(pct, id);
}
static inline void stop_progress(int id) {
  if (!g_no_progress) stop_progress_real(id);
}

// Helper to clear the progress line safely
static inline void progress_clear_line(void) {
  if (!g_no_progress) { usleep(200000); printf("\033[1A\033[2K\r"); fflush(stdout); }
}

// Progress UI gating (set by --no-progress in CLI)
// Note: g_no_progress is declared in packer_defs.h (via compression.h)
#define PROG_START(msg, id) do { if (!g_no_progress) start_progress((msg), (id)); } while (0)
#define PROG_UPDATE(pct, id) do { if (!g_no_progress) update_progress((pct), (id)); } while (0)
#define PROG_STOP(id)        do { if (!g_no_progress) stop_progress((id)); } while (0)
#define PROG_CLEAR_LINE()    do { if (!g_no_progress) { usleep(200000); printf("\033[1A\033[2K\r"); fflush(stdout); } } while (0)
...
int compress_data_with_codec(const char *codec, const unsigned char *input,
  ...
  VPRINTF("[%s%s%s] Compressing %s: %zu bytes...\n",
          PK_ARROW, PK_SYM_ARROW, PK_RES, comp_name, size_to_compress);
  ...
  if (use_apultra) {
    ...
    if (tmp == (size_t)-1) {
      printf("[%s%s%s] %s %s\n", PK_ERR, PK_SYM_ERR, PK_RES, ERR_COMP, "Apultra");
      return 1;
    }
    if (tmp > 0x7fffffff) {
    ...
    VPRINTF(
        "[%s%s%s] ZX7B compression: %zu → %d bytes (%.1f%%)\n",
        PK_INFO, PK_SYM_INFO, PK_RES, size_to_compress, comp_size, (float)comp_size / size_to_compress * 100);
  } else if (use_zx0) {
    ...
    comp_size = (int)out_sz;
    VPRINTF(
        "[%s%s%s] ZX0 compression: %zu → %d bytes (%.1f%%)\n",
        PK_INFO, PK_SYM_INFO, PK_RES, size_to_compress, comp_size, (float)comp_size / size_to_compress * 100);
  } else if (use_blz) {
    ...
    progress_clear_line();
    if (rc != 0 || out == NULL || out_sz == 0 || out_sz > 0x7fffffff) {
      printf("[%s%s%s] %s %s\n", PK_ERR, PK_SYM_ERR, PK_RES, ERR_COMP, "BriefLZ");
      return 1;
    }
    compressed = out;
    comp_size = (int)out_sz;
    ...
    comp_size = out_sz_i;
    VPRINTF(
        "[%s%s%s] EXO compression: %zu → %d bytes (%.1f%%)\n",
        PK_INFO, PK_SYM_INFO, PK_RES, size_to_compress, comp_size, (float)comp_size / size_to_compress * 100);
  } else if (use_pp) {
    ...
    progress_clear_line();
    if (rc != 0 || out == NULL || out_sz == 0 || out_sz > 0x7fffffff) {
      printf("[%s%s%s] %s %s\n", PK_ERR, PK_SYM_ERR, PK_RES, ERR_COMP, "PowerPacker");
      if (out)
        free(out);
      return 1;
    }
    ...
    comp_size = (int)out_sz;
    VPRINTF(
        "[%s%s%s] PowerPacker compression: %zu → %d bytes (%.1f%%)\n",
        PK_INFO, PK_SYM_INFO, PK_RES, size_to_compress, comp_size, (float)comp_size / size_to_compress * 100);
  } else if (use_doboz) {
    ...
    comp_size = rc;
    VPRINTF(
        "[%s%s%s] DoboZ compression: %zu → %d bytes (%.1f%%)\n",
        PK_INFO, PK_SYM_INFO, PK_RES, size_to_compress, comp_size, (float)comp_size / size_to_compress * 100);
  } else if (use_qlz) {
    ...
    if (written == 0 || written > max_out || written > 0x7fffffff) {
      printf("[%s%s%s] %s %s\n", PK_ERR, PK_SYM_ERR, PK_RES, ERR_COMP, "QuickLZ");
      free(out);
      return 1;
    }
    compressed = out;
    ...
    progress_clear_line();
    if (rc != 0 || out == NULL || out_sz == 0 || out_sz > 0x7fffffff) {
      printf("[%s%s%s] %s %s\n", PK_ERR, PK_SYM_ERR, PK_RES, ERR_COMP, "Snappy");
      return 1;
    }
    compressed = out;
    comp_size = (int)out_sz;
    ...
    comp_size = (int)out_sz;
    VPRINTF(
        "[%s%s%s] LZAV compression: %zu → %d bytes (%.1f%%)\n",
        PK_INFO, PK_SYM_INFO, PK_RES, size_to_compress, comp_size, (float)comp_size / size_to_compress * 100);
  } else if (use_lzma) {
    ...
    VPRINTF(
        "[%s%s%s] LZMA compression: %zu → %d bytes (%.1f%%)\n",
        PK_INFO, PK_SYM_INFO, PK_RES, size_to_compress, comp_size, (float)comp_size / size_to_compress * 100);
  } else if (use_zstd) {
    ...
    VPRINTF(
        "[%s%s%s] Zstd compression: %zu → %d bytes (%.1f%%)\n",
        PK_INFO, PK_SYM_INFO, PK_RES, size_to_compress, comp_size, (float)comp_size / size_to_compress * 100);
  } else if (use_shrinkler) {
    ...
    if (!ok || out == NULL || out_sz == 0 || out_sz > 0x7fffffff) {
      printf("[%s%s%s] %s %s\n", PK_ERR, PK_SYM_ERR, PK_RES, ERR_COMP, "Shrinkler");
      if (out)
        free(out);
      return 1;
    }
    ...
    progress_clear_line();
    if (rc == 0 || rc > 0x7fffffff) {
      printf("[%s%s%s] %s %s\n", PK_ERR, PK_SYM_ERR, PK_RES, ERR_COMP, "StoneCracker");
      free(out);
      return 1;
    }
    compressed = out;
    ...
    if (rc == 0 || rc > 0x7fffffff) { // lzsa2_compress returns compressed size,
      ...
      if (rc == 0) { // Assuming 0 is failure if input > 0
        printf("[%s%s%s] %s %s\n", PK_ERR, PK_SYM_ERR, PK_RES, ERR_COMP, "LZSA2");
        free(out);
        return 1;
      }
    }
    ...
    VPRINTF(
        "[%s%s%s] LZ4 compression: %zu → %d bytes (%.1f%%)\n",
        PK_INFO, PK_SYM_INFO, PK_RES, size_to_compress, comp_size, (float)comp_size / size_to_compress * 100);
  }
