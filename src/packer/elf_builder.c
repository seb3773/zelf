...
void write_packed_binary(const char *output_path, int has_pt_interp,
                         ...
                         exe_filter_t g_exe_filter, Elf64_Off stub_off,
  ...
  VPRINTF("[%s%s%s] Final stub: %zu bytes\n", PK_INFO, PK_SYM_INFO, PK_RES, stub_sz);
  ...
  if (use_password) {
    ...
    if (patch_elfz_params_v2(stub_data_final, stub_sz, param_vstart,
      ...
      VPRINTF("[%s%s%s] .elfz_params v2 patched: vstart=0x%lx, paddr=0x%lx, salt=0x%lx\n",
              PK_INFO, PK_SYM_INFO, PK_RES,
              (unsigned long)param_vstart, (unsigned long)param_paddr,
              (unsigned long)pwd_salt);
    } else {
      VPRINTF("[%s%s%s] Warning: .elfz_params v2 not found in stub; no patch applied\n",
    ...
    if (!has_pt_interp) {
      if (patch_elfz_params(stub_data_final, stub_sz, stub_vaddr,
                            packed_vaddr)) {
        VPRINTF("[%s%s%s] .elfz_params patched: virtual_start=0x%lx, packed_data_vaddr=0x%lx\n",
                PK_INFO, PK_SYM_INFO, PK_RES,
                (unsigned long)stub_vaddr, (unsigned long)packed_vaddr);
      } else {
        VPRINTF("[%s%s%s] Warning: .elfz_params not found in STATIC stub; no patch applied\n",
                PK_WARN, PK_SYM_WARN, PK_RES);
      }
    }
  }

  {
    ...
    if (patch_elfz_flags(stub_data_final, stub_sz, base_ver, elfz_flags)) {
      ...
      if (has_pt_interp) {
        ...
        VPRINTF("[%s%s%s] Info: unable to patch .elfz_params flags (dynamic binary)\n",
                PK_INFO, PK_SYM_INFO, PK_RES);
      } else {
  ...
  if (has_pt_interp) {
    ...
    VPRINTF("[%s%s%s] Dynamic entry: 0x%lx (vaddr C_TEXT 0x5000 + stub offset 0x%lx)\n",
            PK_INFO, PK_SYM_INFO, PK_RES, new_ehdr.e_entry, stub_file_offset);
  } else {
    new_ehdr.e_entry = stub_vaddr;
    VPRINTF(
        "[%s%s%s] Static entry (absolute): 0x%lx (stub_vaddr)\n",
        PK_INFO, PK_SYM_INFO, PK_RES, new_ehdr.e_entry);
  }
  ...
  if (has_pt_interp) {
    ...
    int legacy_bcj_layout = (strcmp(stub_codec, "zstd") == 0)
                                ? 1
  ...
  if (predicted_final_size >= actual_size) {
    fprintf(stderr,
            "[%s%s%s] Packing did not reduce size. Packed: %zu bytes >= Original: %zu bytes. Nothing written.\n",
  ...
  long current_pos;
  if (has_pt_interp) {
    VPRINTF("[%s%s%s] dynamic stub.bin already extracted (VIRTUAL_START preconfigured)\n", PK_INFO, PK_SYM_INFO, PK_RES);
  ...
  if (has_pt_interp) {
    pad3 = 0;
    VPRINTF("[%s%s%s] Data immediately after stub (shared offset, no recompute)\n", PK_INFO, PK_SYM_INFO, PK_RES);
  } else {
  ...
  VPRINTF("[%s%s%s] Padding to data: %zu bytes (current_pos: %ld, packed_off: 0x%lx)\n",
  ...
  printf("[%s%s%s] Final size: %ld bytes (vs %zu original)\n",
         PK_ACC1, PK_SYM_DOT, PK_RES, final_size, actual_size);
