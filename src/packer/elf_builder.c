...
int write_packed_binary(
    ...
    uint64_t pwd_salt, uint64_t pwd_obf, Elf64_Half target_machine,
    const unsigned char *density_kexe_tail40) {
  ...
  new_ehdr.e_machine = target_machine;
  new_ehdr.e_version = EV_CURRENT;

  // Compute entry point
  if (has_pt_interp) {
    if (stub_off < (Elf64_Off)ALIGN) {
      ...
      VPRINTF(
          "[%s%s%s] Dynamic entry: 0x%lx (vaddr C_TEXT 0x5000 + stub offset "
          "0x%lx)\n",
          PK_INFO, PK_SYM_INFO, PK_RES, new_ehdr.e_entry, stub_file_offset);
    } else {
      new_ehdr.e_entry = 0x5000;
      VPRINTF("[%s%s%s] Dynamic entry: 0x%lx (vaddr C_TEXT 0x5000)\n", PK_INFO,
  ...
  if (has_pt_interp) {
    ...
    if (target_machine == EM_AARCH64) {
      if (g_exe_filter == EXE_FILTER_KANZIEXE &&
        ...
        legacy_bcj_layout = 0;
      } else if (strcmp(stub_codec, "zstd") == 0 || strcmp(stub_codec, "density") == 0) {
        legacy_bcj_layout = 1;
      } else if ((g_exe_filter == EXE_FILTER_BCJ || g_exe_filter == EXE_FILTER_NONE) &&
                 (strcmp(stub_codec, "lz4") == 0 || strcmp(stub_codec, "apultra") == 0 ||
                  ...
                  strcmp(stub_codec, "shrinkler") == 0 || strcmp(stub_codec, "rnc") == 0 ||
                  strcmp(stub_codec, "lzfse") == 0 || strcmp(stub_codec, "csc") == 0 ||
                  strcmp(stub_codec, "nz1") == 0 || strcmp(stub_codec, "lzsa") == 0 ||
                  strcmp(stub_codec, "lzham") == 0 || strcmp(stub_codec, "stcr") == 0 ||
    ...
    size_t extra_tail40 =
        (density_kexe_tail40 && g_exe_filter == EXE_FILTER_KANZIEXE &&
         strcmp(stub_codec, "density") == 0)
            ? (sizeof(uint32_t) + 40u)
            : 0u;
    size_t packed_block_size =
        marker_len + sizeof(size_t) + sizeof(uint64_t) + sizeof(int) +
        (legacy_bcj_layout ? 0 : sizeof(int)) + extra_tail40 + comp_size;
    size_t ctext_filesz = 0;
    if (stub_off < (Elf64_Off)ALIGN)
      ctext_filesz = 0x100 + stub_sz + packed_block_size;
    else
      ctext_filesz = stub_sz + packed_block_size;
    ...
    packed_segment =
        (Elf64_Phdr){.p_type = PT_LOAD,
                     .p_offset = (stub_off < (Elf64_Off)ALIGN) ? 0 : stub_off,
                     .p_vaddr = 0x5000,
                     .p_paddr = 0x5000,
    ...
    if (target_machine == EM_AARCH64) {
      if (g_exe_filter == EXE_FILTER_KANZIEXE &&
        ...
        legacy_bcj_layout = 1;
                 ...
                 (strcmp(stub_codec, "lz4") == 0 || strcmp(stub_codec, "apultra") == 0 ||
                  ...
                  strcmp(stub_codec, "zx7b") == 0 || strcmp(stub_codec, "pp") == 0 ||
                  strcmp(stub_codec, "qlz") == 0 || strcmp(stub_codec, "snappy") == 0 ||
                  strcmp(stub_codec, "shrinkler") == 0 || strcmp(stub_codec, "lzsa") == 0 ||
                  strcmp(stub_codec, "rnc") == 0 || strcmp(stub_codec, "lzfse") == 0 ||
                  strcmp(stub_codec, "csc") == 0 || strcmp(stub_codec, "nz1") == 0 ||
                  strcmp(stub_codec, "lzham") == 0 || strcmp(stub_codec, "stcr") == 0 ||
                  strcmp(stub_codec, "sc") == 0)) {
        legacy_bcj_layout = 1;
      } else {
      ...
      if (strcmp(stub_codec, "zstd") == 0 || strcmp(stub_codec, "density") == 0) {
        legacy_bcj_layout = 1;
                 ...
                 (strcmp(stub_codec, "lz4") == 0 || strcmp(stub_codec, "apultra") == 0 ||
                  strcmp(stub_codec, "lzav") == 0 || strcmp(stub_codec, "lzma") == 0 ||
                  strcmp(stub_codec, "zstd") == 0 || strcmp(stub_codec, "blz") == 0 ||
                  strcmp(stub_codec, "doboz") == 0 || strcmp(stub_codec, "exo") == 0 ||
                  strcmp(stub_codec, "zx0") == 0 || strcmp(stub_codec, "zx7b") == 0 ||
                  strcmp(stub_codec, "pp") == 0 || strcmp(stub_codec, "qlz") == 0 ||
                  strcmp(stub_codec, "snappy") == 0 || strcmp(stub_codec, "shrinkler") == 0 ||
                  strcmp(stub_codec, "lzsa") == 0 || strcmp(stub_codec, "rnc") == 0 ||
                  strcmp(stub_codec, "lzfse") == 0 || strcmp(stub_codec, "csc") == 0 ||
                  strcmp(stub_codec, "nz1") == 0 || strcmp(stub_codec, "lzham") == 0 ||
                  strcmp(stub_codec, "stcr") == 0 || strcmp(stub_codec, "sc") == 0)) {
        legacy_bcj_layout = 1;
      } else {
        legacy_bcj_layout = 0;
      }
  ...
  if (target_machine == EM_AARCH64) {
    if (g_exe_filter == EXE_FILTER_KANZIEXE &&
        (strcmp(stub_codec, "zstd") == 0 || strcmp(stub_codec, "density") == 0)) {
      legacy_bcj_layout_pre = 0;
    } else if (strcmp(stub_codec, "zstd") == 0 || strcmp(stub_codec, "density") == 0) {
      legacy_bcj_layout_pre = 1;
    } else if ((g_exe_filter == EXE_FILTER_BCJ || g_exe_filter == EXE_FILTER_NONE) &&
               (strcmp(stub_codec, "lz4") == 0 || strcmp(stub_codec, "apultra") == 0 ||
                strcmp(stub_codec, "lzav") == 0 || strcmp(stub_codec, "lzma") == 0 ||
                strcmp(stub_codec, "blz") == 0 || strcmp(stub_codec, "doboz") == 0 ||
  ...
  size_t packed_block_size_pre =
      ...
      (legacy_bcj_layout_pre ? 0 : sizeof(int)) + extra_tail40_pre + comp_size;
  ...
  if (has_pt_interp) {
    ...
    size_t pad_target = (stub_off < (Elf64_Off)ALIGN) ? 0x100 : (size_t)stub_off;
    size_t pad_headers = pad_target - (size_t)current_pos;
    for (size_t i = 0; i < pad_headers; i++) {
      fputc(0, out);
    }
    fwrite(stub_data_final, 1, stub_sz, out);
    VPRINTF("[%s%s%s] Stub written at offset 0x%lx (after %zu-byte padding)\n",
            PK_INFO, PK_SYM_INFO, PK_RES, (unsigned long)pad_target,
            pad_headers);
  } else {
  ...
  size_t packed_block_size = marker_len + sizeof(size_t) + sizeof(uint64_t) +
                             sizeof(int) +
                             (legacy_bcj_layout ? 0 : sizeof(int)) +
                             extra_tail40 + comp_size;
  ...
  if (!legacy_bcj_layout) {
    int filtered_size_int = (int)filtered_size;
    memcpy(cursor, &filtered_size_int, sizeof(int));
    cursor += sizeof(int);
  }

  if (extra_tail40) {
    uint32_t tag = 0x21303454u; /* "T40!" */
    memcpy(cursor, &tag, sizeof(tag));
    cursor += sizeof(tag);
    memcpy(cursor, density_kexe_tail40, 40u);
    cursor += 40u;
  }
