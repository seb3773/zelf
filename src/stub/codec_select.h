...
static inline int lz4_decompress(const char *src, char *dst, int compressedSize,
                                 int dstCapacity) {
  /* Use density_decompress_c for all density stubs (both static and dynamic).
   * For dynamic binaries with stage0, the stage0 wrapper handles NRV2B
   * decompression of the stub separately - this function only decompresses
   * the density-compressed payload data. */
  int r =
      density_decompress_c((const unsigned char *)src, (size_t)compressedSize,
                           (unsigned char *)dst, (size_t)dstCapacity);
  return r >= 0 ? r : -1;
}
