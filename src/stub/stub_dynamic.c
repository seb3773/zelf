...
#else
// Minimal params block for non-password dynamic stub (v1) - for BCJ flag
__attribute__((used, section(".rodata.elfz_params"))) static const unsigned char
    elfz_params_block[] = {
        '+',
        'z',
        ...
        0,
        0,
        0, // version = 1 (flags will be patched at byte 9)
        0,
        0,
        0,
        0,
        ...
        0,
        0,
        0, // packed_data_vaddr (unused for
           // dynamic)
};
#endif
...
static void stub_main_common(unsigned long *original_sp,
  ...
  if (virtual_start < 0x10000) {
    // Robust VMA scan only for codecs that need it (zstd, density).
#if defined(CODEC_ZSTD) || defined(CODEC_DENSITY)
    const unsigned char *best_p = NULL;

#if defined(CODEC_ZSTD) || defined(CODEC_DENSITY)
    // ZSTD/DENSITY-SPECIFIC: For stage0-wrapped binaries, the decompressed stub
    // runs from dst_off (high address), but the packed data is at LOWER
    // addresses in the SAME file mapping. Scan backward within our OWN VMA
    // first.

    // Phase 1: Find OUR VMA and scan backward from current_start_addr to vma_lo
    {
      uint64_t vma_lo = 0, vma_hi = 0;
      if (find_vma_for_addr(current_start_addr, &vma_lo, &vma_hi) == 0 &&
          vma_lo < current_start_addr) {
        // Scan backward: from vma_lo to current_start_addr
        unsigned char *search_start = (unsigned char *)vma_lo;
        unsigned char *search_end = (unsigned char *)current_start_addr;
        ...
        for (unsigned char *p = search_start; p < search_end - 32; p++) {
          if (p[0] == COMP_MARKER[0] && p[1] == COMP_MARKER[1] &&
            ...
            // z_syscall_write(1, "MARKER FOUND (backward)\n", 24);
            uint64_t actual_size = *(uint64_t *)(p + COMP_MARKER_LEN);
            uint32_t comp_size =
                *(uint32_t *)(p + COMP_MARKER_LEN + sizeof(size_t) +
                              sizeof(uint64_t));
...
#endif // CODEC_ZSTD || CODEC_DENSITY

    // Phase 2: Forward scan (fallback if backward scan didn't find marker)
    if (best_p == NULL) {
      ...
      while (vma_scans < 16 && best_p == NULL) {
        ...
        for (unsigned char *p = search_start; p < search_end - 32; p++) {
          if (p[0] == COMP_MARKER[0] && p[1] == COMP_MARKER[1] &&
              p[2] == COMP_MARKER[2] && p[3] == COMP_MARKER[3] &&
              p[4] == COMP_MARKER[4] && p[5] == COMP_MARKER[5]) {
            z_syscall_write(1, "MARKER FOUND\n", 13);
            uint64_t actual_size = *(uint64_t *)(p + COMP_MARKER_LEN);
            uint32_t comp_size =
                *(uint32_t *)(p + COMP_MARKER_LEN + sizeof(size_t) +
                              sizeof(uint64_t));
            ...
            if (actual_size > 1000 && actual_size < 0x20000000 &&
                ...
                (uint64_t)comp_size <
                    ((uint64_t)actual_size * 4ull + 1048576ull)) {
              unsigned char *cand_stream = p + COMP_MARKER_LEN +
...
#ifdef CODEC_ZSTD
              {
                // Legacy ZSTD layout (legacy_bcj_layout=1 in packer) omits the
                // filtered_size field (4 bytes).
                // So the stream starts 4 bytes earlier than cand_stream
                // calculations above (which assume standard layout with
                // filtered_size). Check cand_stream - 4 for Magic.
                // z_syscall_write(1, "Scan Z candidate\n", 17);
                if ((cand_stream - 4) >= search_start) {
                  const unsigned char *p_z = cand_stream - 4;
                  if (p_z[0] == 0x28 && p_z[1] == 0xB5 && p_z[2] == 0x2F &&
                      p_z[3] == 0xFD) {
                    z_syscall_write(1, "Z Magic Found at -4\n", 20);
                    cand_stream -= 4; // Found it! Adjust pointer.
                                      // Adjust payload_sz check if needed?
                    // comp_size was read from offset 22. cand_stream is now
                    // at 26. Matches expectations.
                  } else {
                    // z_syscall_write(1, "Z Magic Miss\n", 13);
                  }
                }
                ...
                if (fcs_cand > 0 && (size_t)fcs_cand > cap_relax) {
                  z_syscall_write(1, "Z Size Fail\n", 12);
                  continue;
                }
                z_syscall_write(1, "Z Candidate Accepted\n", 21);
              }
#endif
...
#else
    // Legacy fixed scan window (small) for non-zstd dynamic stubs.
    const unsigned char *best_p = NULL;
    unsigned char *search_start =
        (unsigned char *)(current_start_addr & ~0xFFFULL);
    unsigned char *search_end = search_start + (256u << 10); // 256KB
    for (unsigned char *p = search_start; p < search_end - 32; p++) {
      if (p[0] == COMP_MARKER[0] && p[1] == COMP_MARKER[1] &&
          p[2] == COMP_MARKER[2] && p[3] == COMP_MARKER[3] &&
          p[4] == COMP_MARKER[4] && p[5] == COMP_MARKER[5]) {
  ...
  int filtered_size = 0;
  // ZSTD/DENSITY/LZHAM_BCJ legacy layout: Data starts immediately after
  // CompressedSize (no FilteredSize)
  ...
  // Check for the extra BCJ flag inserted by packer for ZSTD
  // (legacy flaw: an int (1 or 0) inserted before stream)
  const unsigned char *m = (const unsigned char *)compressed_data;
  if (!(m[0] == 0x28 && m[1] == 0xB5 && m[2] == 0x2F && m[3] == 0xFD)) {
    // If not magic, assume there's a 4-byte flag before it
    // Wait! logic above set compressed_data to offset 26.
    // If flag is present, magic is at 30.
    // But my scanner found magic at offset 26 (cand_stream - 4).
    // So if magic IS at 26, we are good.
    // If magic is NOT at 26, we try +4 (30).
    compressed_data += sizeof(int);
    m = (const unsigned char *)compressed_data;
  }

  if (!(m[0] == 0x28 && m[1] == 0xB5 && m[2] == 0x2F && m[3] == 0xFD))
    z_syscall_exit(4);

  // Relaxed check: Kanzi/Filters may change content size slightly.
  // Don't enforce strict equality with original_size here.
  long long cs = zstd_peek_content_size((const unsigned char *)compressed_data,
                                        (size_t)compressed_size);
  // Only check if size is drastically wrong or error
  if (cs < 0) {
    // Error or unknown. ZSTD frames should have size in our usage?
    // If unknown (0), we proceed. If error, we might fail later.
  }
#endif
...
// For Shrinkler, BriefLZ, Snappy: use padding
...
#ifdef CODEC_BRIEFLZ
    dst_cap = (int)filtered_size; // BriefLZ uses filtered size for exact output
#endif
    decomp_result =
  ...
  {
#if defined(CODEC_ZSTD)
    // For ZSTD: Check if Kanzi filter was actually applied (Bit 1 of elfz_flags
    // -> Bit 9 of version) This allows support for filter=none by skipping
    // unfiltering.
    int kanzi_applied = 0;
    {
      uint64_t pb_addr;
      __asm__ volatile("lea elfz_params_block(%%rip), %0" : "=r"(pb_addr));
      const unsigned char *pb = (const unsigned char *)pb_addr;
      // Valid params block signature: "+zELF-PR"
      if (pb[0] == '+' && pb[1] == 'z' && pb[2] == 'E' && pb[3] == 'L' &&
          pb[4] == 'F' && pb[5] == '-' && pb[6] == 'P' && pb[7] == 'R') {
        uint64_t params_version = *(const uint64_t *)(pb + 8);
        kanzi_applied = ((params_version >> 9) & 1ULL) ? 1 : 0;
      }
    }
...
#else
    // For NON-ZSTD: Keep original logic (unconditional unfilter/copy)
    // to preserve compatibility with existing working stubs (as per user
    // instruction). Note: For filter=none on non-ZSTD, checks elsewhere might
    // handle pass-through.
    void *unfiltered = (void *)z_syscall_mmap(
        NULL, (size_t)original_size, PROT_READ | PROT_WRITE,
        MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
