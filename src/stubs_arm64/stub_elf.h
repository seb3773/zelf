...
static inline int find_vma_for_addr(uint64_t addr, uint64_t *out_start,
                                    uint64_t *out_end) {
  int fd = (int)z_syscall_open("/proc/self/maps", 0, 0);
  if (fd < 0)
    return -1;
  ...
  for (int i = 0; i < n; i++) {
    if (buf[i] == '\n' || i == n - 1) {
      ...
      int has_x = 0;
      if (line_len > 0 &&
          parse_maps_line_range(buf + line_start, line_len, &s, &e, &has_x) ==
              0) {
        if (addr >= s && addr < e) {
          *out_start = s;
          *out_end = e;
...
static inline int find_vma_ending_at(uint64_t end_addr, uint64_t *out_start,
                                     uint64_t *out_end) {
  int fd = (int)z_syscall_open("/proc/self/maps", 0, 0);
  if (fd < 0)
    return -1;
...
// NOTE: name kept for compatibility with existing stub core.
static inline void *make_hatch_x86_64(Elf64_Phdr const *const phdr,
  ...
  if (phdr->p_type == PT_LOAD && (phdr->p_flags & PF_X)) {
    hatch = (uint32_t *)z_syscall_mmap(0, 0x1000,
