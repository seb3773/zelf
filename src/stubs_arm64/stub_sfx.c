...
static inline long z_syscall_rmdir(const char *pathname) {
  ...
  register long x0 asm("x0") = (long)AT_FDCWD;
  register long x1 asm("x1") = (long)pathname;
  register long x2 asm("x2") = AT_REMOVEDIR;
  asm volatile("svc #0" : "+r"(x0) : "r"(x1), "r"(x2), "r"(x8) : "memory");
  return x0;
}
...
static int join2(const char *a, const char *b, char *out, size_t cap) {
  size_t la = (size_t)simple_strlen(a);
  size_t lb = (size_t)simple_strlen(b);
  if (la + 1 + lb + 1 > cap)
    return -1;
  simple_memcpy(out, a, la);
  if (la && out[la - 1] != '/') {
    out[la++] = '/';
  }
  simple_memcpy(out + la, b, lb);
  out[la + lb] = '\0';
  return 0;
}
...
static unsigned is_ascii_octal(char c) {
  return (c >= '0' && c <= '7');
}
...
static int tar_extract(const uint8_t *tar, size_t tar_size, const char *root,
  ...
  while (off + 512 <= tar_size) {
    ...
    const char *size_field = (const char *)(hdr + 124);
    ...
    uint64_t fsize = parse_octal(size_field, 12);

    char full[4096];
    if (name_len + 1 > sizeof(full))
      return -1;

    if (join2(root, name, full, sizeof(full)) != 0)
      return -1;
      ...
      if (fd < 0 && fd == -EEXIST) {
        if (policy == 0) {
          write_cstr(STDOUT_FILENO,
                     "File exists. Overwrite? [a]ll/[y]es/[n]o/[q]uit: ");
          char ans[16];
    ...
    size_t blocks = (size_t)((fsize + 511u) / 512u);
    off += 512 + blocks * 512;
  }
  return 0;
}

static int write_out_file(const char *out_path, const void *data, size_t n,
                          int *overwrite_policy) {
  int policy = overwrite_policy ? *overwrite_policy : 0;
...
long elfz_sfx_entry(void) {
  ...
  uint64_t map_off = footer.payload_off & ~((uint64_t)0xFFFu);
  uint64_t delta = footer.payload_off - map_off;
  uint64_t map_sz = (footer.payload_size + delta + 0xFFFu) & ~((uint64_t)0xFFFu);
  if (map_sz == 0)
    map_sz = 4096u;

  void *map0 = (void *)z_syscall_mmap(NULL, (size_t)map_sz, PROT_READ, MAP_PRIVATE,
  ...
  const uint8_t *end = p + (size_t)footer.payload_size;
  ...
  if (marker[0] != COMP_MARKER[0] || marker[1] != COMP_MARKER[1] ||
    ...
    (void)z_syscall_munmap(map0, (size_t)map_sz);
    (void)z_syscall_close((int)fd);
    return 1;
  }
  ...
  if ((size_t)name_len > sizeof(footer.name) - 1)
    return 1;
  ...
  char stored_name[64];
  simple_memcpy(stored_name, p, name_len);
  ...
  size_t out_map_sz = (size_t)((orig_size + 0xFFFu) & ~((uint64_t)0xFFFu));
  if (out_map_sz == 0)
    out_map_sz = 4096u;
  ...
  if (is_tar) {
    ...
    int rc = tar_extract((const uint8_t *)outbuf, (size_t)orig_size, root, &policy);
    if (rc == 0) {
      write_cstr(STDOUT_FILENO, "\nDone: ");
  ...
  int rc = write_out_file(root, outbuf, (size_t)orig_size, &policy);
