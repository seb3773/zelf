// elfz_probe.c - Extract features from ELF64 binaries for BCJ vs KanziEXE
// prediction Platform: Linux x86_64 Language: C99 Notes:
// - CPU-bound scanning of segments; avoids external deps.
// - Parses ELF header and Program Headers only (section headers may be
// stripped).
// - Measures entropy, branch opcode densities, padding, ASCII ratios,
// zero-runs, etc.
...
static const char *CSV_HEADER =
    ...
    "riprel_estimate,nop_ratio_text,imm64_mov_cnt,align_pad_ratio,ro_ptr_like_"
    "cnt,"
    "rel32_intext_ratio,rel32_intext_cnt,rel32_disp_entropy8,"
    "x_load_cnt,ro_load_cnt,rw_load_cnt,bss_sz,avg_p_align_log2,max_p_align_"
    "log2,"
    "shnum,sh_exec_cnt,sh_align_avg_log2,"
    "dyn_needed_cnt,dynsym_cnt,pltrel_cnt,dynrel_text_cnt,dynrel_text_density_"
    "per_kb,"
    "ret_cnt,rel_branch_ratio,avg_rel32_abs,max_rel32_abs,rel32_target_unique_"
    "cnt,"
...
static inline int vaddr_to_off(uint64_t vaddr, uint64_t *off_out,
  ...
  for (size_t i = 0; i < phnum; ++i) {
    ...
    if (vaddr >= va && vaddr < va + mem) {
      ...
      if (off < file_off || off >= file_off + file_sz)
...
// Estimate RIP-relative addressing patterns: 0x48 0x8B/0x8D with ModRM mod=00
// r/m=101
...
static inline double ascii_ratio(const uint8_t *p, size_t n) {
  ...
  for (size_t i = 0; i < n; ++i) {
    uint8_t b = p[i];
    if ((b >= MIN_PRINTABLE_ASCII && b <= MAX_PRINTABLE_ASCII) || b == '\t' ||
        b == '\n' || b == '\r')
      c++;
  }
  return (double)c / (double)n;
...
static uint64_t parse_dynrel_count(const uint8_t *base, size_t fsz,
  ...
  for (size_t i = 0; i < phnum; ++i) {
    ...
    for (size_t j = 0; j < cnt; ++j) {
      ...
      if (tag == DT_RELASZ)
        ...
        rela_ent = val ? val : rela_ent;
      else if (tag == DT_RELSZ)
        rel_sz = val;
      else if (tag == DT_RELENT)
        rel_ent = val ? val : rel_ent;
    }
...
static void scan_buffer_features(const uint8_t *buf, size_t len,
                                 uint64_t hist[256], uint64_t *zeros,
                                 uint64_t *nonzeros) {
...
static int probe_one(const char *path) {
  ...
  if (fsz < eh->e_phoff + eh->e_phnum * sizeof(Elf64_Phdr)) {
    munmap(map, fsz);
    return -1;
  }
  ...
  uint64_t bss_sz = 0;
  uint64_t p_align_sum_log2 = 0, p_align_max_log2 = 0;
  uint64_t p_align_cnt = 0;
  ...
  // For text code-specific counters we need concatenated view: we simply count
  // across each text segment
  uint64_t e8_cnt = 0, e9_cnt = 0, ff_cj_cnt = 0, eb_cnt = 0, jcc32_cnt = 0,
           riprel_cnt = 0, imm64_cnt = 0;
  uint64_t text_nop_bytes =
      0; // computed via ratio function separately per segment
  ...
  uint64_t rel32_abs_sum = 0;
  uint64_t rel32_abs_max = 0;
  // Cap unique targets to avoid O(N^2) and memory blowup
  uint64_t uniq_targets[4096];
  int uniq_n = 0;
  ...
  for (uint16_t i = 0; i < eh->e_phnum; ++i) {
    ...
    if (ph[i].p_memsz > ph[i].p_filesz)
      bss_sz += (uint64_t)(ph[i].p_memsz - ph[i].p_filesz);
    // p_align stats
    if (ph[i].p_align) {
      uint32_t lg = ilog2_u64(ph[i].p_align);
      p_align_sum_log2 += lg;
      p_align_cnt++;
      if (lg > p_align_max_log2)
        p_align_max_log2 = lg;
    }
    ...
    if (is_x) {
      ...
      for (size_t j = 0; j + 4 < sz; ++j) {
        ...
        if (b == 0xE8 || b == 0xE9) {
          ...
          for (int xs = 0; xs < xseg_n; ++xs) {
            if ((uint64_t)target >= xseg[xs].lo &&
                (uint64_t)target < xseg[xs].hi) {
              rel32_intext++;
              break;
            }
          }
          ...
          if (uniq_n < (int)(sizeof(uniq_targets) / sizeof(uniq_targets[0]))) {
            ...
            int seen = 0;
            for (int ui = 0; ui < uniq_n; ++ui) {
              if (uniq_targets[ui] == t) {
                seen = 1;
                break;
              }
            }
            if (!seen) {
              uniq_targets[uniq_n++] = t;
            }
          }
        }
      }
      ...
      // Accumulate nop count and pad bytes explicitly to compute ratio on total
      // text later
      for (size_t j = 0; j < sz; ++j)
        text_nop_bytes += (p[j] == 0x90);
    ...
    if (ro_sz > 0) {
      for (uint16_t i = 0; i < eh->e_phnum; ++i) {
        ...
        if (is_r && !is_w && !is_x) {
          ...
          for (size_t j = 0; j < sz; ++j) {
            uint8_t b = p[j];
            if ((b >= MIN_PRINTABLE_ASCII && b <= MAX_PRINTABLE_ASCII) ||
                b == '\t' || b == '\n' || b == '\r')
              ro_ascii_bytes++;
          }
        }
  ...
  // Re-count RET in text (single pass over LOAD exec segments to keep branch
  // loop light)
  uint64_t ret_cnt = 0;
  for (uint16_t i = 0; i < eh->e_phnum; ++i) {
    if (ph[i].p_type != PT_LOAD)
    ...
    for (uint64_t j = 0; j < sz; ++j) {
      ...
      if (b == 0xC3)
        ret_cnt++;
      else if (b == 0xC2 && j + 2 < sz)
        ret_cnt++; // treat as RET
    }
  }

  uint64_t branch_raw = e8_cnt + e9_cnt + ff_cj_cnt;
  double branch_density =
      (text_sz > 0) ? ((double)branch_raw / ((double)text_sz / 1024.0)) : 0.0;
  double nop_ratio =
      (text_sz > 0) ? ((double)text_nop_bytes / (double)text_sz) : 0.0;
  double pad_ratio =
      (text_sz > 0) ? ((double)text_pad_bytes / (double)text_sz) : 0.0;
  ...
  double rel_branch_ratio =
      (rel_b + abs_b) ? (double)rel_b / (double)(rel_b + abs_b) : 0.0;
  ...
  if (ro_sz > 0) {
    for (uint16_t i = 0; i < eh->e_phnum; ++i) {
      ...
      int is_x = (flags & PF_X) != 0;
      if (is_r && !is_w && !is_x)
        ro_ptr_like_cnt += ro_ptr_like(p, (size_t)sz);
    }
  }
  ...
  uint64_t dyn_needed_cnt = 0, dynsym_cnt = 0, pltrel_cnt = 0,
           dynrel_text_cnt = 0;
  for (size_t i = 0; i < eh->e_phnum; ++i) {
    ...
    for (size_t j = 0; j < cnt; ++j) {
      ...
      if (tag == DT_NEEDED)
        ...
        dt_pltrel = val; // DT_REL or DT_RELA
        ...
        dt_relaent = val ? val : dt_relaent;
      else if (tag == DT_REL)
        dt_rel = val;
      else if (tag == DT_RELSZ)
        dt_relsz = val;
      else if (tag == DT_RELENT)
        dt_relent = val ? val : dt_relent;
    }
    (void)dt_jmprel; // silence: set but not used
    // dynsym count via DT_HASH (nbucket,nchain)
    if (dt_hash) {
      uint64_t h_off = 0;
      if (vaddr_to_off(dt_hash, &h_off, base, fsz, ph, eh->e_phnum)) {
        if (h_off + 8 <= fsz) {
    ...
    if (dt_pltrelsz) {
      ...
      if (ent)
        pltrel_cnt = dt_pltrelsz / ent;
    }
    // Count relocations targeting exec segments (RELA)
    if (dt_rela && dt_relasz && dt_relaent) {
      uint64_t r_off = 0;
      if (vaddr_to_off(dt_rela, &r_off, base, fsz, ph, eh->e_phnum)) {
        size_t n = (size_t)(dt_relasz / dt_relaent);
        for (size_t k = 0; k < n; ++k) {
          if (r_off + (k + 1) * dt_relaent > fsz)
            break;
          const Elf64_Rela *re =
              (const Elf64_Rela *)(base + r_off + k * dt_relaent);
          uint64_t ro = re->r_offset;
          for (int xs = 0; xs < xseg_n; ++xs) {
            if (ro >= xseg[xs].lo && ro < xseg[xs].hi) {
              dynrel_text_cnt++;
              break;
            }
          }
    ...
    if (dt_rel && dt_relsz && dt_relent) {
      uint64_t r_off = 0;
      if (vaddr_to_off(dt_rel, &r_off, base, fsz, ph, eh->e_phnum)) {
        size_t n = (size_t)(dt_relsz / dt_relent);
        for (size_t k = 0; k < n; ++k) {
          if (r_off + (k + 1) * dt_relent > fsz)
            break;
          const Elf64_Rel *re =
              (const Elf64_Rel *)(base + r_off + k * dt_relent);
          uint64_t ro = re->r_offset;
          for (int xs = 0; xs < xseg_n; ++xs) {
            if (ro >= xseg[xs].lo && ro < xseg[xs].hi) {
              dynrel_text_cnt++;
  ...
  double rel32_intext_ratio =
      (e8e9_total ? (double)rel32_intext / (double)e8e9_total : 0.0);
  double disp8_entropy = entropy_from_hist(disp8_hist, e8e9_total);
  double avg_rel32_abs =
      (e8e9_total ? (double)rel32_abs_sum / (double)e8e9_total : 0.0);
  ...
  for (uint16_t i = 0; i < eh->e_phnum; ++i) {
    ...
    for (uint64_t j = 0; j < sz; ++j) {
      ...
      if (have_prev)
        sumprod += (long double)b * (long double)prev;
      prev = b;
      have_prev = 1;
      n_text++;
    }
  ...
  printf(
      ...
      "%llu,%.6f,%.6f,%llu,%llu,"
      "%.6f,%llu,%llu,%llu\n",
      path, (size_t)fsz, etype, has_interp, n_load, stripped,
      (unsigned long long)dynrel_cnt, (unsigned long long)text_sz,
      (unsigned long long)ro_sz, (unsigned long long)data_sz, text_ratio,
      ro_ratio, data_ratio, text_ent, ro_ent, data_ent, zeros_ratio_total,
      (unsigned long long)zero_runs_16, (unsigned long long)zero_runs_32,
      ascii_ratio_ro, (unsigned long long)e8_cnt, (unsigned long long)e9_cnt,
      (unsigned long long)ff_cj_cnt, (unsigned long long)eb_cnt,
      (unsigned long long)jcc32_cnt, branch_density,
      (unsigned long long)riprel_cnt, nop_ratio, (unsigned long long)imm64_cnt,
      pad_ratio, (unsigned long long)ro_ptr_like_cnt, rel32_intext_ratio,
      (unsigned long long)rel32_intext, disp8_entropy,
      (unsigned long long)x_load_cnt, (unsigned long long)ro_load_cnt,
      (unsigned long long)rw_load_cnt, (unsigned long long)bss_sz,
      (p_align_cnt ? (double)p_align_sum_log2 / (double)p_align_cnt : 0.0),
      (double)p_align_max_log2, (unsigned)(eh->e_shnum), 0u, 0.0,
      (unsigned long long)dyn_needed_cnt, (unsigned long long)dynsym_cnt,
      (unsigned long long)pltrel_cnt, (unsigned long long)dynrel_text_cnt,
      (text_sz ? ((double)dynrel_text_cnt / ((double)text_sz / 1024.0)) : 0.0),
      (unsigned long long)ret_cnt, rel_branch_ratio, avg_rel32_abs,
      (unsigned long long)rel32_abs_max, (unsigned long long)uniq_n,
      autocorr1_text, (unsigned long long)max_run00,
      (unsigned long long)max_run90, (unsigned long long)max_run_any_len);
