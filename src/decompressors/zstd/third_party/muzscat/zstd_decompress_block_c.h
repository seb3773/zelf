...
static void ZSTD_buildFSETable_body_bmi2(
    ZSTD_seqSymbol *dt, const short *normalizedCounter, unsigned maxSymbolValue,
    const U32 *baseValue, const U32 *nbAdditionalBits, unsigned tableLog,
    void *wksp, size_t wkspSize) {
  ZSTD_buildFSETable_body(dt, normalizedCounter, maxSymbolValue, baseValue,
      ...
      ""); /* minimum possible size: 1 byte for symbol encoding types */
  {
    ...
    /* Build DTables */
    {
      size_t const llhSize = ZSTD_buildSeqTable(
          ...
          ddictIsCold, nbSeq, dctx->workspace, sizeof(dctx->workspace),
...
FORCE_INLINE_TEMPLATE size_t DONT_VECTORIZE ZSTD_decompressSequences_body(
  ...
  if (nbSeq) {
    ...
    RETURN_ERROR_IF(BIT_reloadDStream(&seqState.DStream) <
...
FORCE_INLINE_TEMPLATE size_t ZSTD_prefetchMatch(size_t prefetchPos,
  ...
  prefetchPos += sequence.litLength;
  {
    ...
    const BYTE *const match =
        ...
        sequence
            .offset; /* note : this operation can overflow when seq.offset is
                      * really too large, which can only happen when input is
                      * corrupted. No consequence though : memory address is
                      * only used for prefetching, not for dereferencing */
    PREFETCH_L1(match);
    PREFETCH_L1(match +
