...
size_t ZSTD_freeDCtx(ZSTD_DCtx *dctx) {
  ...
  RETURN_ERROR_IF(ZSTD_DCtx_get_staticSize(zds), memory_allocation,
...
size_t ZSTD_getFrameHeader_advanced(ZSTD_frameHeader *zfhPtr, const void *src,
  ...
  ZSTD_memset(zfhPtr, 0,
              sizeof(*zfhPtr)); /* not strictly necessary, but static analyzer
                                   do not understand that zfhPtr is only going
                                   to be read only if return value is zero,
                                   since they are 2 different signals */
  ...
  while (1) {
    ...
    case bt_compressed:
      decodedSize = ZSTD_decompressBlock_internal(
          dctx, op, (size_t)(oend - op), ip, cBlockSize, /* frame */ 1);
      break;
    case bt_raw:
      decodedSize = ZSTD_copyRawBlock(op, (size_t)(oend - op), ip, cBlockSize);
      break;
    case bt_rle:
      decodedSize = ZSTD_setRleBlock(op, (size_t)(oend - op), *ip,
  ...
  while (someMoreWork) {
    ...
    case zdss_loadHeader:
      ...
      /* Adapt buffer sizes to frame header instructions */
      {
        ...
        size_t const neededOutBuffSize =
            ZSTD_DCtx_get_outBufferMode(zds) == ZSTD_bm_buffered
...

        {
          ...
          if (tooSmall || tooLarge) {
            ...
            if (ZSTD_DCtx_get_staticSize(zds)) { /* static DCtx */
              DEBUGLOG(4, "staticSize : %u",
                       (U32)ZSTD_DCtx_get_staticSize(zds));
              assert(ZSTD_DCtx_get_staticSize(zds) >=
                     sizeof(ZSTD_DCtx)); /* controlled at init */
              RETURN_ERROR_IF(bufferSize > ZSTD_DCtx_get_staticSize(zds) -
                                               sizeof(ZSTD_DCtx),
                              memory_allocation, "");
            } else {
              ...
              zds->outBuffSize = 0;
              zds->inBuff =
                  (char *)ZSTD_customMalloc(bufferSize, ZSTD_defaultCMem);
