...
#define MAP_ANONYMOUS 0x20
static inline void *elfz_sys_mmap(size_t length) {
  register long x8 asm("x8") = 222;                      /* __NR_mmap */
  register long x0 asm("x0") = 0;                        /* addr */
  register long x1 asm("x1") = (long)length;             /* length */
  register long x2 asm("x2") = PROT_READ | PROT_WRITE;   /* prot */
  register long x3 asm("x3") = MAP_PRIVATE | MAP_ANONYMOUS; /* flags */
  register long x4 asm("x4") = -1;                       /* fd */
  register long x5 asm("x5") = 0;                        /* offset */
  asm volatile("svc 0"
               : "+r"(x0)
...
static inline long elfz_sys_munmap(void *addr, size_t length) {
  register long x8 asm("x8") = 215;          /* __NR_munmap */
...
#include <unistd.h>
static inline void *elfz_sys_mmap(size_t length) {
  long r = syscall(SYS_mmap, (void *)0, length, PROT_READ | PROT_WRITE,
