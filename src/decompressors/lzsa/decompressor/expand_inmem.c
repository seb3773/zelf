 ...
 */
size_t lzsa_get_max_decompressed_size_inmem(const unsigned char *pFileData,
                                            size_t nFileSize) {
  ...
  while (pCurFileData < pEndFileData) {
    ...
    if ((pCurFileData + nFrameSize) > pEndFileData ||
        lzsa_decode_frame(pCurFileData, nFrameSize, &nBlockDataSize,
                          &nIsUncompressed) != 0)
      return -1;
 ...
 * @param pFormatVersion pointer to format version, updated if this function is
 * successful
 *
 * @return actual decompressed size, or -1 for error
 */
size_t lzsa_decompress_inmem(unsigned char *pFileData,
                             unsigned char *pOutBuffer, size_t nFileSize,
                             size_t nMaxOutBufferSize,
                             const unsigned int nFlags, int *pFormatVersion) {
  ...
  if (nFlags & LZSA_FLAG_RAW_BLOCK) {
    return (size_t)lzsa_decompressor_expand_block(
        pFileData, (int)nFileSize, pOutBuffer, 0, (int)nMaxOutBufferSize,
        *pFormatVersion, nFlags);
  }
  ...
  while (pCurFileData < pEndFileData) {
    ...
    if (!nIsUncompressed) {
      ...
      nDecompressedSize = lzsa_decompressor_expand_block(
          pCurFileData, nBlockDataSize, pOutBuffer,
          (int)(pCurOutBuffer - pOutBuffer),
          (int)(pEndOutBuffer - pCurOutBuffer), *pFormatVersion, nFlags);
