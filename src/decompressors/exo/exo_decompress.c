...
#endif
static inline __attribute__((always_inline)) void _mm_prefetch(const char *p, int hint) {
    (void)hint;
#if defined(ELFZ_STUB_ARM64)
    (void)p;
#else
...
static inline __attribute__((always_inline)) void exo_memcpy(unsigned char *dst,
...
static inline __attribute__((always_inline)) void exo_memset(unsigned char *dst,
                                                            unsigned char v,
                                                            int len) {
#if defined(ELFZ_STUB_ARM64)
    for (int i = 0; i < len; i++) {
...
int exo_decompress(const unsigned char *in, int in_size,
    ...
    while (1) {
        ...
        
        {
            ...
            if (offset >= len) {
                exo_memcpy(dst, src, len);
                c.out_pos += len;
            } else if (offset == 1) {
                /* RLE case: repeat single byte */
                exo_memset(dst, *src, len);
                c.out_pos += len;
            } else {
    ...
    copy_lit:
        if (len >= 8) {
            ...
            exo_memcpy(c.out + c.out_pos, c.in + c.in_pos, len);
            c.out_pos += len;
            c.in_pos += len;
        } else {
            unsigned char *dst = c.out + c.out_pos;
