...
2. **Locate payload**:
   ...
   - *PIE*: scan its own VMA for the magic marker (e.g., `zELFl4` for LZ4).
...
zELF supports 16 algorithms. Adding a codec follows a standard path.

### 4.1 Codec list
LZ4 (default/fast), LZMA (max ratio/slow), ZSTD (balanced), Apultra, ZX7B, ZX0, BriefLZ, Exomizer, PowerPacker, Snappy, Doboz, QuickLZ, LZAV, Shrinkler, StoneCracker, LZSA2.
...
**Standard layout** (KanziEXE or NONE filter, except ZSTD):
| Field | Size |
|-------|------|
| COMP_MARKER | 6 bytes |
...
**Legacy layout** (ZSTD, or BCJ filter with any codec):
| Field | Size |
|-------|------|
| COMP_MARKER | 6 bytes |
| original_size | 8 bytes |
