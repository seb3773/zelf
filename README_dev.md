...
zELF is a modular packer for Linux ELF64 **x86_64 and AArch64 (arm64)**. The architecture cleanly separates packing logic (host/userland) from loading logic (nostdlib stub injected into the packed binary).
...
3. **Filtering**: Transform machine code bytes to improve compression (BCJ or KanziEXE). Choice driven by ML decision trees or forced by the user.
4. **Compression**: The filtered stream is compressed with one of the 22 supported codecs.
5. **Encapsulation**:
...
│   │   ├── compression.c        # Single facade to the 22 codecs
...
│   │
│   ├── stub/            # Runtime loader (nostdlib target, x86_64)
...
│   │
│   ├── stubs_arm64/     # Runtime loader (nostdlib target, AArch64)
│   │   ├── start.S              # ASM entry point (_start)
│   │   ├── stub_static.c        # Static-binary logic
│   │   ├── stub_dynamic.c       # PIE/Dynamic logic
│   │   ├── stub_dynexec.c        # Dynexec logic (ET_EXEC-style loader)
...
- **Position Independent Code (PIC)**: The stub may be loaded anywhere; all addressing is RIP-relative (x86_64) or PC-relative (AArch64).
...
### Multi-arch packer builds (x86_64 + Aarch64)
The packer binary is host-arch specific (you build either an x86_64 packer or an Aarch64 packer), but it can support packing/unpacking both architectures **if** the corresponding stub assets are present in `build/`.

Required asset directories:
- `build/stubs_x86_64/`: x86_64 stub `.bin` files.
- `build/stubs_arm64/`: Aarch64 stub `.bin` files.

Stage0 assets (x86_64-generated):
Stage0 assets are stored **per architecture**:
- `build/stage0_x86_64/stage0_nrv2b_le32_x86_64.bin`
- `build/stage0_x86_64/stage0_nrv2b_le32_offsets_x86_64.h`
- `build/stage0_arm64/stage0_nrv2b_le32_arm64.bin`
- `build/stage0_arm64/stage0_nrv2b_le32_offsets_arm64.h`

Notes:
- Some codecs use a stage0 wrapper (host-side packing feature). If the target stage0 assets are missing, the packer will still compile, but those stage0-wrapped codecs will be unavailable for that target.
- At build time, `make packer` prints a status line about cross-arch support depending on whether the other-arch stubs are detected.
...
Cleaning targets:
- `make clean`: cleans host build artifacts and the host stub directory only (`build/stubs_$(HOST_ARCH)`), keeping any other-arch stubs in place.
- `make clean-keep-stubs`: same as `clean` but preserves **all** stub directories and stage0 artifacts (useful to assemble a full multi-arch release tree).
