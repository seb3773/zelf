...
# Density stub size constraint: Density also has a large stub (~24KB), making it
# impractical for small binaries. We exclude binaries < 50KB for Density as well.
DENSITY_MIN_BINARY_SIZE = 50 * 1024  # 50KB in bytes
...
def apply_codec_size_exclusions(comp_name: str, df: pd.DataFrame) -> pd.DataFrame:
    ...
    if comp_name == 'density':
        res = res[res['file_size'] >= DENSITY_MIN_BINARY_SIZE]
    if comp_name == 'exo':
...
class CompressorAnalyzer:
    ...
    def calculate_compression_ratios(self):
        ...
        for comp_name, df in self.compressors.items():
            ...
            if 'bcj_ratio' in df.columns and 'kanzi_ratio' in df.columns:
                ...
                # Density exclusion: filter out binaries < 50KB for Density due to large stub size
                if comp_name == 'density' and 'file_size' in df.columns:
                    df = df[df['file_size'] >= DENSITY_MIN_BINARY_SIZE].copy()
        ...
        print("      Density excludes binaries < 50KB due to large stub size (~24KB)")
        print("      These codecs are reserved for binaries >= 50KB where stub overhead is negligible")
        print()
    ...
    def _plot_size_range_performance(self):
        ...
        plt.suptitle('Top 10 compressors by binary size range\n(Note: ZSTD and Density excluded for binaries < 50KB due to large stubs ~20-25KB)',
    ...
    def _plot_performance_curves(self):
        ...
        for comp_name, df in sorted(self.compressors.items()):
            ...
            # Density exclusion: filter out binaries < 50KB for Density
            if comp_name == 'density' and 'file_size' in df.columns:
                df = df[df['file_size'] >= DENSITY_MIN_BINARY_SIZE].copy()
