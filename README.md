# zELF — x86_64 + AArch64 ELF packer
<p align="center">
  <img src="logo_zelf.jpg" alt="zELF logo" width="200">
</p>


zELF is an ELF64 packer for Linux **x86_64 and AArch64 (arm64)**, inspired by UPX but with a modular structure: 22 compression codecs, the ability to integrate your own codecs, two executable filters (BCJ, KanziEXE) and automatic selection via ML models. It handles static and dynamic/PIE binaries, provides a `.zlf` archive mode (with optional self-extracting SFX output), and brute-force helper modes (`-best`, `-crazy`) that explore multiple combinations to maximize ratio.
...
- Archive SFX mode: build a self-extracting executable (`--archive-sfx`, `--archive-tar-sfx`).
...
## Platform and limits
- Targets Linux ELF64 **x86_64** and **AArch64 (arm64)**. ELF 32-bit is not supported.
...
### Multi-arch model (host vs target)
zELF is **multi-arch** in two distinct senses:

- **Packer binary (host arch)**
  - `build/zelf` can be compiled on **x86_64** or **arm64**.
- **Packed binary (target arch)**
  - zELF can pack **x86_64 ELF64** binaries and **AArch64 ELF64** binaries.

Important constraint:

- **Stubs “foreign” required**
  - To pack a given architecture, the packer must embed the matching runtime assets:
    - the target stubs (`build/stubs_x86_64/` or `build/stubs_arm64/`)
    - and when applicable, the target stage0 blobs (`build/stage0_x86_64/` or `build/stage0_arm64/`).

Practical consequences (no cross-compiling assumed):

- If you compile the packer on **arm64**, you can generate arm64 stubs locally. To pack **x86_64** binaries, you must have **pre-generated x86_64 stubs/stage0** in the tree (typically produced by a build on x86_64).
- If you compile the packer on **x86_64**, same logic: you can generate x86_64 stubs locally. To pack **arm64** binaries, you must have **pre-generated arm64 stubs/stage0** in the tree (typically produced by a build on arm64).

The `--version` output prints the host architecture and the detected supported targets for packing.
...
### Clean behavior (multi-arch safe)
`make clean` is intentionally **non-destructive across architectures**:

- It cleans the current host build artifacts and the host stub directory.
- It keeps any pre-generated “foreign” stubs/stage0, because they cannot be regenerated on this host without cross-compiling.

If you want to keep everything (including all stubs and stage0) while cleaning object files, use `make clean-keep-stubs`.
