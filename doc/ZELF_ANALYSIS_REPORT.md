...
- **Direct mode**: static binaries (known absolute addresses), read from `.rodata`.
- **VMA scan mode**: PIE binaries (ASLR); the stub scans its own memory via `/proc/self/maps` or direct signature search (6-byte marker + metadata validation).
