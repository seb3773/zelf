...
| Syscall | Syscall # | Purpose | File |
|---------|----------|---------|------|
| `read` | arch-specific | Read password from TTY | stub_syscalls.h |
| `write` | arch-specific | Writes (memfd, prompts) | stub_syscalls.h |
| `open` / `openat` | arch-specific | Open `/dev/tty`, ld.so | stub_syscalls.h |
| `close` | arch-specific | Close fd | stub_syscalls.h |
| `fstat` | arch-specific | ld.so file size | stub_syscalls.h |
| `mmap` | arch-specific | Allocate memory, map segments | stub_syscalls.h |
| `mprotect` | arch-specific | Change segment permissions | stub_syscalls.h |
| `munmap` | arch-specific | Free temporary memory | stub_syscalls.h |
| `ioctl` | arch-specific | Disable TTY echo (password) | stub_syscalls.h |
| `exit` | arch-specific | Exit on error | stub_syscalls.h |
| `memfd_create` | arch-specific | Fast-path ET_EXEC (ZSTD) | stub_syscalls.h |
| `execveat` | arch-specific | Fast-path ET_EXEC (ZSTD) | stub_syscalls.h |

Notes:
- x86_64 stubs typically use `syscall` ABI and `open`.
- AArch64 stubs use AArch64 syscall ABI and typically use `openat` instead of `open`.

### 2.2 Call pattern

```c
// stub_syscalls.h - inline asm wrapper (x86_64 example)
...
for (size_t i = 0; i < rela_count; i++) {
    if (ELF64_R_TYPE(rela[i].r_info) == R_X86_64_RELATIVE /* x86_64 */ ||
        ELF64_R_TYPE(rela[i].r_info) == R_AARCH64_RELATIVE /* arm64 */) {
...
// Example: "/lib64/ld-linux-x86-64.so.2" (x86_64) or "/lib/ld-linux-aarch64.so.1" (AArch64)
