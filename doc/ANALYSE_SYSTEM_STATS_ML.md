...
### 3.2 Practical pitfalls (training + integration)

- **Python environment sanity**:
  - Before training, ensure the venv is consistent:
    ```bash
...
- **Generated predictor headers**:
  - The exported `src/packer/<codec>_predict_dt.h` may call `memset`. Ensure `<string.h>` is included before any `*_predict_dt.h` include in the packer TU.

- **Symbol collisions across codecs**:
  - Always export with `--symbol-prefix <codec>`.
  - Ensure the typedef is `<codec>_CodecFeat` and the entrypoint is `<codec>_dt_predict_from_pvec(const double *in)`.

- **AUTO evaluation (SKIP avoidance)**:
  - The evaluator relies on a codec-specific log line. Ensure the packer emits:
    - `Heuristic <CODEC> filter choice: %s chosen`

- **AUTO invariants**:
  - Do not modify the stub selection logic when adding predictors. AUTO must select the filter and the packer must re-select the appropriate stub for the chosen filter (required for correct runtime behavior and consistent sizes).
